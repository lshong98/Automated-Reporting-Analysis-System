...
app.post('/getUnsubmitted', function(req, res) {
    ...
    var sql = "SELECT CONCAT(z.zoneCode, a.areaCode) AS area, s.staffName AS staff, DATE_FORMAT(SUBDATE(CURDATE(), 3), '%Y-%m-%d') AS date FROM tblarea a JOIN tblstaff s ON a.staffID = s.staffID JOIN tblzone z ON a.zoneID = z.zoneID WHERE a.areaID NOT IN (SELECT r.areaID FROM tblreport r WHERE r.creationDateTime >= SUBDATE(CURDATE(), 3) AND r.creationDateTime <= CURDATE()) AND (a.collection_frequency LIKE CONCAT('%', DATE_FORMAT(SUBDATE(CURDATE(), 3), '%a'), '%')) UNION SELECT CONCAT(z.zoneCode, a.areaCode) AS area, s.staffName AS staff, DATE_FORMAT(SUBDATE(CURDATE(), 2), '%Y-%m-%d') AS date FROM tblarea a JOIN tblstaff s ON a.staffID = s.staffID JOIN tblzone z ON a.zoneID = z.zoneID WHERE a.areaID NOT IN (SELECT r.areaID FROM tblreport r WHERE r.creationDateTime >= SUBDATE(CURDATE(), 3) AND r.creationDateTime <= CURDATE()) AND (a.collection_frequency LIKE CONCAT('%', DATE_FORMAT(SUBDATE(CURDATE(), 2), '%a'), '%')) UNION SELECT CONCAT(z.zoneCode, a.areaCode) AS area, s.staffName AS staff, DATE_FORMAT(SUBDATE(CURDATE(), 1), '%Y-%m-%d') AS date FROM tblarea a JOIN tblstaff s ON a.staffID = s.staffID JOIN tblzone z ON a.zoneID = z.zoneID WHERE a.areaID NOT IN (SELECT r.areaID FROM tblreport r WHERE r.creationDateTime >= SUBDATE(CURDATE(), 3) AND r.creationDateTime <= CURDATE()) AND (a.collection_frequency LIKE CONCAT('%', DATE_FORMAT(SUBDATE(CURDATE(), 1), '%a'), '%')) UNION SELECT CONCAT(z.zoneCode, a.areaCode) AS area, s.staffName AS staff, DATE_FORMAT(SUBDATE(CURDATE(), 0), '%Y-%m-%d') AS date FROM tblarea a JOIN tblstaff s ON a.staffID = s.staffID JOIN tblzone z ON a.zoneID = z.zoneID WHERE a.areaID NOT IN (SELECT r.areaID FROM tblreport r WHERE r.creationDateTime >= SUBDATE(CURDATE(), 3) AND r.creationDateTime <= CURDATE()) AND (a.collection_frequency LIKE CONCAT('%', DATE_FORMAT(SUBDATE(CURDATE(), 0), '%a'), '%'))";
