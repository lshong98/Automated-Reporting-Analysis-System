...
app.post('/editBinDatabase', function(req, res){
    ...
    function makeImage (image, ID, myTimeStamp) {
        var base64Image = image.split(';base64,').pop();
        var extension = image.split(';base64,')[0].split('/')[1];
        var image_path = '/' + ID + '-' + myTimeStamp +'.' + extension;
        var local_store_path = 'images/bin-database'+ image_path,
            public_url = 'https://storage.googleapis.com/' + bucketName + '/' + local_store_path;
    ...
    var writtenOff = req.body.writtenOff.replace(/'/g,"\\\\'");  
    var id = req.body.id;
    var myTimeStamp = req.body.myTimeStamp
    ...
    if (pic !== '' && pic.substring(0,4) == 'data') {
        pic = makeImage(pic, serialNo, myTimeStamp);
...
app.post('/editBdbFromHist', function(req, res){
    'use strict';
    ...
    database.query(editSql, function(err, result) {
        if (err) {
            ...
            console.log("INSERT INTO TBLBDB");
            database.query(sql, function(err2, result2){
                if(err2){
                    throw err2;
                }else{
                    console.log("INSERT INTO LOG");
                    res.json({"status": "success", "message": "Updates has been modified"});
                }
            })
        }        
    });
