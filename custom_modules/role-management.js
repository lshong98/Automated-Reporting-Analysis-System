...
app.get('/getAllRole', function (req, res) {
    ...
    var sql = "SELECT positionID AS id, positionName AS name, (CASE WHEN positionStatus = 'A' THEN 'ACTIVE' WHEN positionStatus = 'I' THEN 'INACTIVE' END) AS status FROM tblposition";
    database.query(sql, function (err, result) {
        if (err) {
            throw err;
        }
...
app.get('/getPositionList', function (req, res) {
    ...
    var sql = "SELECT positionID AS id, positionName AS name FROM tblposition WHERE positionStatus = 'A'";
    database.query(sql, function (err, result) {
        if (err) {
            throw err;
        } else {
