    ...
    f.waterfallQuery("SELECT tbldcs.dcsID FROM tbldcs WHERE tbldcs.areaID LIKE '%" + req.body.area + "%' AND CURDATE() BETWEEN tbldcs.periodFrom and tbldcs.periodTo").then(function (dcs) {
        if (dcs == null) {
            ...
            if (req.body.todayday == "sun") {
                ...
                var sql = "SELECT tbluser.name, tblacr.mon, tblacr.tue, tblacr.wed, tblacr.thu, tblacr.fri, tblacr.sat FROM tbluser JOIN tblacr ON tbluser.userID = tblacr.customerID WHERE tblacr.dcsID LIKE '%" + dcs.dcsID + "%' AND tblacr." + req.body.todayday + " = 1";
